...
namespace Grids2D.Geom {
	...
	public class VoronoiFortune {
		...
		public VoronoiCell[] cells; // this is the output
...
//				});
			...
			eventQueue.Sort ((Event e1, Event e2) => {
				if (e1.x<e2.x - Point.PRECISION) return -1;
				else if (e1.x>e2.x + Point.PRECISION) return 1;
				else if (e1.p.y<e2.p.y - Point.PRECISION) return -1;
				else if (e1.p.y>e2.p.y + Point.PRECISION) return 1; 
				else return 0;
			});
		}
...
//				}
		...
		bool GetNearestSegmentPointToCorner(Point corner, List<Segment> segments, bool leftOrRightSides, out Point nearest) {
			...
			for (int k=0;k<segments.Count;k++) {
				...
				p = s.start;
				if ( Point.EqualsBoth(p, corner)) continue;
		...
		Point CropPoint(Point point, Segment s, VoronoiCell cell) {
			...
			if (DoublesAreEqual(point.x, X0)) cell.left.Add (point);
			if (DoublesAreEqual(point.x, X1)) cell.right.Add (point);
			if (DoublesAreEqual(point.y, Y0)) cell.bottom.Add (point);
			if (DoublesAreEqual(point.y, Y1)) cell.top.Add (point);
			return point;
		}
