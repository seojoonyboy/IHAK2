...
namespace Grids2D
{
	...
	public partial class Grid2D : MonoBehaviour
	{
		...
		/// </summary>
		public GameObject TerritoryToggle (int territoryIndex, bool visible, Color color, bool refreshGeometry, Texture2D texture)
		{
			return TerritoryToggle (territoryIndex, visible, color, texture, Misc.Vector2one, Misc.Vector2zero, 0);
		}

		/// <summary>
		/// Colorize specified region of a territory by indexes.
		/// </summary>
		public GameObject TerritoryToggle (int territoryIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation)
		{
			if (territories == null || territoryIndex < 0 || territoryIndex >= territories.Count)
				return null;
