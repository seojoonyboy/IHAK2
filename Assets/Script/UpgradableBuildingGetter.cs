...
public class UpgradableBuildingGetter : MonoBehaviour {
    ...
    private void OnMyBuildingsAdded(Enum Event_Type, Component Sender, object Param) {
        List<GameObject> buildingInfos = (List<GameObject>)Param;
    }
    ...
    private void OnResourceChange(Enum Event_Type, Component Sender, object Param) {
        ...
        canInvoke = false;
        List<GameObject> upgradableBuildings = GetUpgradableBuildingList();
    }
    ...
    public Resource CalcCost(int lv, int rarerity) {
        ...
        int foodCost = Convert.ToInt32(250.0f * (rarerity + lv) / 2.0f);
        int goldCost = Convert.ToInt32(250.0f * (rarerity + lv) / 2.0f);
        int envCost = Convert.ToInt32(20.0f * (rarerity + lv) / 2.0f);
...
//public class MyEqualityComparere : IEqualityComparer<GameObject> {
//    public bool Equals(GameObject x, GameObject y) {
//        BuildingObject buildingObject_x = x.GetComponent<BuildingObject>();
//        BuildingObject buildingObject_Y = y.GetComponent<BuildingObject>();
//        return buildingObject_x.data.card.id == buildingObject_Y.data.card.id;
//    }
