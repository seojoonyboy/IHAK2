...
public class MenuSceneController : MonoBehaviour {
    ...
    void Start() {
        ...
        switchButtons.transform.GetChild(0).GetComponent<Button>().OnClickAsObservable().ThrottleFirst(TimeSpan.FromMilliseconds(410)).Subscribe(_ => switchButton(true));
        switchButtons.transform.GetChild(1).GetComponent<Button>().OnClickAsObservable().ThrottleFirst(TimeSpan.FromMilliseconds(410)).Subscribe(_ => switchButton(false));
        ...
        dragStream.Where(_ => mousDownPosition - Input.mousePosition.x < -500).ThrottleFirst(TimeSpan.FromMilliseconds(410)).Subscribe(_ => switchButton(true));
        dragStream.Where(_ => mousDownPosition - Input.mousePosition.x > 500).ThrottleFirst(TimeSpan.FromMilliseconds(410)).Subscribe(_ => switchButton(false));
    }
